...
function salvar_filme($titulo, $genero, $ano)
{
	...
	$sql = "INSERT INTO acervos (titulo, genero, ano) 
...
// função para buscar todos os filmes do acervo
function buscar_acervos()
{
	$conn = conectar();
...
// função para exibir acervos
function exibir_acervos()
{
	$result = buscar_acervos();

	// se o valor de result for nulo, retornamos uma mensagem de erro
	if ($result == null) {
		$retorno = '<h3>Não há acervos cadastrados</h3>';
...
function deletar_filme($id_filme)
{
	...
	$sql = "DELETE FROM acervos WHERE id_filme = $id_filme";
...
function buscar_filme($id_filme)
{
	...
	$sql = "SELECT * FROM acervos WHERE id_filme = $id_filme";
...
function editar_filme($id_filme, $titulo, $genero, $ano)
{
	...
	$sql = "UPDATE acervos SET titulo = '$titulo', genero = '$genero', ano = '$ano' 
